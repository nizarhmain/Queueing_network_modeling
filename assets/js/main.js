...
(function () {
  ...
  var getRandomInt = function (min, max) {
	...
	var newMoleculeSimulation = function (newMolecule, example) {
		...
		Messenger().post({
		  message: 'Nuova Simulazione QNAP',
		  type: 'success',
		  showCloseButton: true,
		  hideAfter: 2
		});
    ...
    newMoleculeSimulation (qns_data, 'zebi');
	});

	// QNS simulation adaptation
		      ...
		      .each(function(d) {
			      ...
			      d3.select(this)
		      		...
		      		.style("fill", function(d) { 
								...
								let symbol_type = d.symbol.split(" ")
								console.log(symbol_type[0]);
								
								if(symbol_type[0] === 'Router') {
									return "#FF6347"
								}

								if(symbol_type[0] === 'Host') {
									return "#93FF33"
								}

								if(symbol_type[0] === 'Terminal') {
									return "#33CEFF"
								}
