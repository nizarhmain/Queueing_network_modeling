  ...
  */
  ...
  window.main = (function() {
    ...
    */
    ...
    svg = d3version3.select('.network').append('svg').attr('width', width).attr('height', height);
    /* ZOOM and PAN
    */
    /* create container elements
    */
    container = svg.append('g');
    container.call(d3version3.behavior.zoom().scaleExtent([0.5, 8]).on('zoom', (function() {
      return global.vis.attr('transform', "translate(" + d3version3.event.translate + ")scale(" + d3version3.event.scale + ")");
    })));
    ...
    */
    global.vis.append('rect').attr('class', 'overlay').attr('x', -500000).attr('y', -500000).attr('width', 1000000).attr('height', 1000000).on('click', (function(d) {
      ...
      */      global.selection = null;
      d3version3.selectAll('.node').classed('selected', false);
      return d3version3.selectAll('.link').classed('selected', false);
    }));
    /* END ZOOM and PAN
    */
    global.colorify = d3version3.scale.category10();
    /* initialize the force layout
    */
    global.force = d3version3.layout.force().size([width, height]).charge(-400).linkDistance(60).on('tick', (function() {
    ...
    */
    d3version3.select(window).on('keydown', function() {
      if (d3version3.event.keyCode === 46) {
    ...
    */
    ...
    global.vis.on('mousemove.add_link', (function(d) {
      ...
      */
      ...
      if (global.new_link_source != null) {
        ...
        */
        p = d3version3.mouse(global.vis.node());
    ...
    return d3version3.selectAll('.tool').on('click', function() {
      var new_tool, nodes;
      d3version3.selectAll('.tool').classed('active', false);
      d3version3.select(this).classed('active', true);
  ...
  update = function() {
    ...
    */
    ...
    new_nodes = nodes.enter().append('g').attr('class', 'node').on('click', (function(d) {
      /* SELECTION
      */      global.selection = d;
      d3version3.selectAll('.node').classed('selected', function(d2) {
        return d2 === d;
      });
      return d3version3.selectAll('.link').classed('selected', false);
    }));
    ...
    links.enter().insert('line', '.node').attr('class', 'link').on('click', (function(d) {
      /* SELECTION
      */      global.selection = d;
      d3version3.selectAll('.link').classed('selected', function(d2) {
        return d2 === d;
      });
      return d3version3.selectAll('.node').classed('selected', false);
    }));
    links.exit().remove();
    ...
    */
    ...
    new_nodes.append('circle').attr('r', 18).attr('stroke', function(d) {
      ...
      return d3version3.hcl(global.colorify(d.type)).brighter(3);
    });
    /* draw the label
    */
  ...
  drag_add_link = function(selection) {
    return selection.on('mousedown.add_link', (function(d) {
      ...
      */
      return d3version3.event.preventDefault();
    })).on('mouseup.add_link', (function(d) {
